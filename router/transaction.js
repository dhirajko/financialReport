...
router.get("/", auth, async (req, res) => {
  if (req.query.accountId) {
    ...
    const transaction = await readTransactionsByAccountId(
      req.user.id,
      req.query.accountId
    );
...
router.post("/directTransaction", auth, async (req, res) => {
  req.body.user = req.user.id;
  const transaction = await combineTransaction(req.body);
